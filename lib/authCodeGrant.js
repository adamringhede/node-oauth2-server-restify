...
function checkParams (next) {
  ...
  // Response type
  this.responseType = body.responseType || query.responseType;
  if (this.responseType !== 'code') {
    return next(new error.BadMethodError('Invalid responseType parameter (must be "code")'));
  }

  // Client
  this.clientId = body.clientId || query.clientId;
  if (!this.clientId) {
    return next(new error.BadMethodError('Invalid or missing clientId parameter'));
  }
...
function generateCode (next) {
  var self = this;
  token(this, 'authorizationCode', function (err, code) {
    self.authCode = code;
    next(err);
  });
}
