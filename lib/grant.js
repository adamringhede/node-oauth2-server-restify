...
/**
 * Basic request validation and extraction of grantType and client creds
 *
 * @param  {Function} next
 * @this   OAuth
 */
function extractCredentials (next) {
  ...
  // Grant type
  this.grantType = this.req.body && this.req.body.grantType;
  if (!this.grantType || !this.grantType.match(this.config.regex.grantType)) {
    return next(new error.BadMethodError('Invalid or missing grantType parameter'));
  }
  ...
  if (!this.client.clientId ||
    ...
    return next(new error.InvalidCredentialsError('Invalid or missing clientId parameter'));
  } else if (!this.client.clientSecret) {
    return next(new error.InvalidCredentialsError('Missing clientSecret parameter'));
  }
...
function credsFromBody (req) {
  return new Client(req.body.clientId, req.body.clientSecret);
}
...
/**
 * Delegate to the relvant grant function based on grantType
 *
 * @param  {Function} next
 * @this   OAuth
 */
function checkGrantType (next) {
  ...
  switch (this.grantType) {
    ...
    default:
      next(new error.MissingParameterError('Invalid grantType parameter or parameter missing'));
  }
}

/**
 * Grant for authorizationCode grant type
 *
 * @param  {Function} next
 */
...
/**
 * Grant for refreshToken grant type
 *
 * @param  {Function} next
 */
function useRefreshTokenGrant (next) {
  var token = this.req.body.refreshToken;

  if (!token) {
    return next(new error.BadMethodError('No "refreshToken" parameter'));
  }
...
/**
 * Grant for clientCredentials grant type
 *
 * @param  {Function} next
 */
function useClientCredentialsGrant (next) {
  ...
  if (!clientId || !clientSecret) {
    return next(new error.InvalidCredentialsError('Missing parameters. "clientId" and "clientSecret" are required'));
  }
...
function useExtendedGrant (next) {
  ...
  this.model.extendedGrant(this.grantType, this.req,
      function (err, supported, user) {
        ...
        if (!supported) {
          return next(new error.BadMethodError('Invalid grantType parameter or parameter missing'));
...
function checkGrantTypeAllowed (next) {
  this.model.grantTypeAllowed(this.client.clientId, this.grantType,
      function (err, allowed) {
        ...
        if (!allowed) {
          return next(new error.InvalidCredentialsError('The grant type is unauthorised for this clientId'));
        }
...
function generateRefreshToken (next) {
  if (this.config.grants.indexOf('refreshToken') === -1) return next();
...
function sendResponse (next) {
  ...
  if (this.refreshToken) response.refreshToken = this.refreshToken;
